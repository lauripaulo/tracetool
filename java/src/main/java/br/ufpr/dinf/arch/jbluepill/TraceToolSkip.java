...
public class TraceToolSkip {
	
	private static org.apache.logging.log4j.Logger logger = LogManager.getFormatterLogger("TraceTool");

	// 1 Billion
	public static long INSTRUCTIONS_SKIP = 10000000000L;
	// 2 Billion after skip number
	public static long INSTRUCTION_LIMIT = 12000000000L;
	// 5.000.000 Print hint
	public static long PRINT_HINT_SIZE   = 10000000L;

//	public static long INSTRUCTIONS_SKIP = 1000000L;
//	public static long INSTRUCTION_LIMIT = 30000000L;
//	public static long PRINT_HINT_SIZE   = 10000L;
...
//		savestate();
	...
	public void processLine(String line) throws IOException {
		...
		// skip instructions
		if (instructionCounter < TraceToolSkip.INSTRUCTIONS_SKIP) {
			printHint ++;
			if (printHint > PRINT_HINT_SIZE) {
				printHint = 0;
				float percent = ( ((float)(instructionCounter)) / TraceToolSkip.INSTRUCTIONS_SKIP) * 100;
				logger.info(
						String.format("Skip instructions progress: %2.4f%% - Instructions processed: %d", 
						percent, instructionCounter));
			}
		} else if (instructionCounter == TraceToolSkip.INSTRUCTIONS_SKIP){
			fileOutWriter.println(buffer.toString());
			instructionsInFileCounter = 0;
			logger.info(String.format("Skipped %s instructions from valgrind. Starting to collect traces.", 
					instructionCounter));
		} else { 
	...
	private void createNewOutputFile() throws IOException
	{
		...
		currentLogFile = String.format("%s-trace-%05d.txt",
		outputTraceFileName, fileNumberCounter);
	...
	public static void main(String[] args) {
		...
		System.out.println("--------------------");
		System.out.println("Reads trace lines from stdin, parses it and store in configurable");
		System.out.println("files with configurable max instructions per file.");
		System.out.println("");
		System.out.println("Usage: TraceTool --mode:stdin|socket|file --compress --readfile:<filename> --max:<lines> --outputfile:<filename> --xmldata:<filename>");
		System.out.println();
		TraceToolSkip tool = new TraceToolSkip();
		try {
			...
			if (!"stdin".equals(tool.getWorkingMode()) && !"file".equals(tool.workingMode) && !"socket".equals(tool.getWorkingMode())) {
				System.out.println("Param --mode:stdin|socket|file is wrong. See usage. Unknow mode: " + tool.getWorkingMode());
				System.exit(4);
			}
