...
public class TraceToolSimulator {
    ...
    public TraceToolSimulator() {
        ...
        final SegmentBuffer sb128 = new SegmentBuffer(1, 128);
        ...
        final TranslateLookasideBuffer tlb32 = new TranslateLookasideBuffer(1, 32);
        final TranslateLookasideBuffer tlb64 = new TranslateLookasideBuffer(1, 64);
        final TranslateLookasideBuffer tlb128 = new TranslateLookasideBuffer(1, 128);
        final TranslateLookasideBuffer tlb16x8 = new TranslateLookasideBuffer(16, 8);
        tlbList.add(tlb32);
        tlbList.add(tlb64);
        tlbList.add(tlb128);
        tlbList.add(tlb16x8);
    ...
    public void start() throws InvalidTraceLineException, IOException {
        ...
        if (state.isTraceFilesCompressed() && state.isNonCompressedFilesRemoved()) {
            ...
            for (String zipFile : state.getCompressedTraceFiles()) {
                ...
                String tempFilePath = System.getProperty("java.io.tmpdir") + File.separator + "tracetool-"
                                + UUID.randomUUID() + "-tempfile.txt";
    ...
    private void saveResults(boolean isFinal) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        for (TranslateLookasideBuffer tlb : tlbList) {
            buffer.append(String.format("%n%s%n", TraceUtils.buildCacheResults(tlb, true, rogueMemRefsList.size())));
        }

        for (SegmentBuffer sb : sbList) {
            buffer.append(String.format("%n%s%n", TraceUtils.buildCacheResults(sb, true, rogueMemRefsList.size())));
        }
    ...
    public void processTraceFile(String traceFile) throws InvalidTraceLineException, IOException {
        ...
        worker.processTrace(new InstructionObserver() {
            ...
            private void handleRogueMemReference(Instruction asm, MemoryMapEntry memEntry) {
                String logMessage = String.format("Rogue memory reference! Asm=%s, Entry=%s", asm, memEntry);
    ...
    private String getProgressDetails() {
        ...
        String progress = String.format(
                        "Instructions readed: %s ( %3.2f %% ) - Rogue: %s - Sim. segs. created: %s - Time: %s seconds. ",
    ...
    public static void main(String[] args) {
        ...
        System.out.println("-----------------------------");
        System.out.println("");
        System.out.println("Usage: TraceToolSimulator --xmlfile:<file> --memmap:<file> "
                        + "--csvexport:<csvfile> --results:<filename>");
        System.out.println();
        TraceToolSimulator tool = new TraceToolSimulator();
        try {
